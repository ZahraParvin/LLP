...
void cache_init() {
  ...
  uint32_t number_blocks = cache_size / block_size;

  cache = (uint8_t*)calloc(number_blocks, block_size); // filling cache array with zeros
}
...
void cache_access(access_t accesstype, uint32_t address) {

  uint32_t index = address / block_size; // calculating index and block offest for cache
  uint32_t offset = address % block_size;

  if (cache[index]) { // checking cache is valid or not and if it's a miss update it as valid
    cache_statistics.hits++;
  } else {
    cache[index] = 1;
  }

  cache_statistics.accesses++; // 1 sucessful access
}
...
void main(int argc, char** argv) {
  ...
  while (1) {
    ...
    /* Do a cache access */
    // ADD YOUR CODE HERE
    cache_access(access.accesstype,access.address);
  }
